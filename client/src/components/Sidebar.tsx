...
const Sidebar = ({ isSidebarOpen, onCloseSidebar }: SidebarProps = {}) => {
  ...
  // Effect to sync external state with internal state
  useEffect(() => {
  ...
  // Determine visibility class based on mobile or desktop
  const sidebarVisibilityClass = isMobile
    ? isOpen ? 'translate-x-0' : '-translate-x-full'
    : '';

  return (
    <aside className={`${isMobile ? 'fixed inset-y-0 left-0 z-30 w-72 transition-transform duration-300 ease-in-out ' + sidebarVisibilityClass : 'w-64 relative'} bg-white border-r border-neutral-200 flex flex-col h-full shrink-0`}>
      <div className="p-4 border-b border-neutral-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <svg className="w-7 h-7 text-primary-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            ...
            <h1 className="text-lg font-bold font-heading text-neutral-900">Knowledge Atlas</h1>
          </div>
          ...
          <ul>
            <li>
              <Link href="/" 
                ...
                className={`flex items-center px-4 py-2 text-neutral-900 ${isActive("/") ? "bg-neutral-100 rounded-md font-medium" : "hover:bg-neutral-100 rounded-md"}`}>
            ...
            <li>
              <Link 
                href="/progress" 
                onClick={handleLinkClick}
                className={`flex items-center px-4 py-2 text-neutral-600 ${isActive("/progress") ? "bg-neutral-100 rounded-md font-medium" : "hover:bg-neutral-100 rounded-md"} mt-1`}>
            ...
            <li>
              <Link 
                href="/study-list" 
                onClick={handleLinkClick}
                className={`flex items-center px-4 py-2 text-neutral-600 ${isActive("/study-list") ? "bg-neutral-100 rounded-md font-medium" : "hover:bg-neutral-100 rounded-md"} mt-1`}>
                <svg className="w-5 h-5 mr-3 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              {domains.map((domain, index) => (
                <li key={domain}>
                  <Link 
                    href={`/domain/${domain}`} 
                    onClick={handleLinkClick}
                    className={`flex items-center px-4 py-2 text-neutral-600 hover:bg-neutral-100 rounded-md ${index > 0 ? 'mt-1' : ''}`}>
